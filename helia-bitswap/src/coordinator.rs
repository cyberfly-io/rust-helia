...
pub struct Bitswap {
    ...
    /// Shared outbound sender slot used by Network facade
    outbound_sender_slot: Arc<RwLock<Option<tokio::sync::mpsc::UnboundedSender<OutboundMessage>>>>,
    /// Connected peers
...
impl Bitswap {
    ...
    pub async fn new(blockstore: Arc<dyn Blocks>, config: BitswapConfig) -> Result<Self> {
        ...
        // Shared outbound sender slot between coordinator and network facade
        let outbound_sender_slot = Arc::new(RwLock::new(None));
        ...
        // Create wantlist with a separate Network instance sharing the same sender slot
        let network_for_wantlist = Arc::new(Network::new(
            config.network.clone(),
            outbound_sender_slot.clone(),
        ));
    ...
    pub async fn set_outbound_sender(
        &mut self,
        ...
        {
            let mut slot = self.outbound_sender_slot.write().await;
            *slot = Some(tx.clone());
        }

        // Wantlist network shares the same slot, so it will see the sender automatically
