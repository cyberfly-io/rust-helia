use helia_ipns::{IpnsRouting, IpnsRecord};
use helia_ipns::routing::{DhtRouter, PutOptions, GetOptions};
use helia_ipns::record::{marshal_record_protobuf, unmarshal_record_protobuf, sign_record, verify_signature};
...
fn create_test_swarm(keypair: Keypair) -> Result<(libp2p::Swarm<Kademlia<MemoryStore>>, PeerId), Box<dyn std::error::Error>> {
...
async fn test_dht_router_creation() {
    ...
    let (swarm, peer_id) = create_test_swarm(keypair)
    ...
    println!("Created DHT router with user-provided libp2p swarm, peer ID: {}", peer_id);
}
...
async fn test_dht_put_operation() {
    ...
    let keypair = Keypair::generate_ed25519();
    let (swarm, peer_id) = create_test_swarm(keypair.clone())
        .expect("Failed to create swarm");
    ...
    // Test put operation
    let result = router.put(&routing_key, &marshaled, PutOptions::default()).await;
