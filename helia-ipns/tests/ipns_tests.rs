...
async fn test_publish_basic() {
    ...
    assert!(publish_result
        .record
        .value
        .contains("bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi"));
}
...
async fn test_resolve_published_record() {
    ...
    let publish_result = name
        .publish("test-key-resolve", &cid, pub_options)
        .await
        .unwrap();
...
async fn test_publish_increments_sequence() {
    ...
    let result1 = name
        .publish("test-seq-key", &cid1, options.clone())
        .await
        .unwrap();
...
async fn test_unpublish() {
    ...
    // Publish
    let result = name
        .publish("test-unpublish-key", &cid, options.clone())
        .await
        .unwrap();
...
async fn test_nocache_option() {
    ...
    let publish_result = name
        .publish("test-nocache-key", &cid, pub_options)
        .await
        .unwrap();
    ...
    res_options.offline = true;
    let result1 = name
        .resolve(&publish_result.public_key, res_options.clone())
        .await;
...
async fn test_signature_generation() {
    ...
    assert!(
        !result.record.signature.is_empty(),
        "V1 signature should not be empty"
    );
    assert!(
        result.record.signature_v2.is_some(),
        "V2 signature should exist"
    );
    assert!(
        !result.record.signature_v2.unwrap().is_empty(),
        "V2 signature should not be empty"
    );
}
...
async fn test_signature_verification_valid() {
    ...
    use helia_ipns::record::{verify_signature, IpnsRecord};
...
async fn test_signature_verification_wrong_key() {
    ...
    assert!(
        result.is_err(),
        "Wrong routing key should fail verification"
    );
}
...
async fn test_publish_creates_valid_signatures() {
    ...
    assert!(
        verify_result.is_ok(),
        "Published record should have valid signature"
    );
}
...
async fn test_select_best_record() {
    ...
    assert_eq!(
        best_idx, 1,
        "Should select record with highest sequence number"
    );
}
...
async fn test_protobuf_marshal_unmarshal_roundtrip() {
    use helia_ipns::record::{
        marshal_record_protobuf, sign_record, unmarshal_record_protobuf, IpnsRecord,
    };
    use libp2p_identity::Keypair;
...
async fn test_protobuf_signature_verification() {
    ...
    assert!(
        result.is_ok(),
        "Signature should be valid after protobuf roundtrip"
    );
}
...
async fn test_protobuf_with_dag_cbor() {
    use helia_ipns::record::{
        decode_cbor_data, encode_cbor_data, marshal_record_protobuf, sign_record,
        unmarshal_record_protobuf, IpnsRecord,
    };
    use libp2p_identity::Keypair;
