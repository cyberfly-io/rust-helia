...
async fn test_trustless_gateway_fetch_real() {
    ...
    let result = gateway.retrieve(cid, BlockRetrievalOptions::default()).await;
...
async fn test_trustless_gateway_invalid_cid() {
    ...
    let cid = Cid::try_from("bafybeiabc123invalidcidinvalidcidinvalidcidinvalid").unwrap_or(Cid::default());
