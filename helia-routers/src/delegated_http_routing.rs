...
impl DelegatedHTTPRouter {
    ...
    async fn query_endpoint(&self, endpoint: &Url, cid: &Cid) -> Result<Vec<ProviderInfo>, RoutingError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(RoutingError::RoutingFailed(format!("HTTP {}", response.status())));
        }
        ...
        debug!("Found {} provider records from {}", routing_response.providers.len(), url);
        
        // Convert provider records to ProviderInfo
        let mut providers = Vec::new();
        for record in routing_response.providers.iter().take(self.config.max_providers) {
            // Parse peer ID
        ...
        match router.find_providers(&cid).await {
            Ok(providers) => {
                ...
                for provider in &providers {
                    println!("  Provider: {} with {} addrs", provider.peer_id, provider.addrs.len());
                }
            }
            Err(e) => {
