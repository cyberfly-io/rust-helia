...
impl HTTPGatewayRouter {
    ...
    fn gateway_to_multiaddr(gateway: &Url) -> Option<Multiaddr> {
        ...
        let port = gateway.port().unwrap_or(if gateway.scheme() == "https" { 443 } else { 80 });
        let protocol = if gateway.scheme() == "https" { "https" } else { "http" };
...
impl ContentRouting for HTTPGatewayRouter {
    async fn find_providers(&self, cid: &Cid) -> Result<Vec<ProviderInfo>, RoutingError> {
        debug!("HTTPGatewayRouter: Returning {} gateways as providers for {}", self.gateways.len(), cid);
