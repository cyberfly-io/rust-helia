    ...
    R: AsyncRead + Unpin,
{
    ...
    async fn read_varint(&mut self) -> Result<u64> {
        ...
        loop {
            ...
            match self.reader.read_exact(&mut buf[bytes_read..bytes_read + 1]).await {
                Ok(_) => {},
                Err(e) => {
                    ...
                    return Err(HeliaError::other(format!("Failed to read varint byte: {}", e)));
                }
            }
    ...
    pub async fn read_header(&mut self) -> Result<CarHeader> {
        ...
        if length == 0 || length > 1024 * 1024 {
            return Err(HeliaError::other(format!("Invalid header length: {}", length)));
        }
    ...
    pub async fn read_block(&mut self) -> Result<Option<CarBlock>> {
        ...
        let length = match self.read_varint().await {
            ...
            Err(e) => {
                ...
                if err_str.contains("early eof") || err_str.contains("failed to fill whole buffer") || err_str.contains("UnexpectedEof") {
        ...
        if length > 100 * 1024 * 1024 {
            return Err(HeliaError::other(format!("Block too large: {} bytes", length)));
        }
        
        // Read the entire section (CID + data)
        let mut section = vec![0u8; length];
        self.reader.read_exact(&mut section).await
